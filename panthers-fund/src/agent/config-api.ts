...
export function handleConfigRequest(
  ...
  const knownRoutes = ['rpc', 'strategy', 'trading-limits', 'pause', 'tee-measurements', 'code-update'];
...
function dispatchConfigRoute(
  ...
  switch (route) {
    ...
    case 'strategy':
      return handleStrategy(data, deps);
    case 'trading-limits':
      return handleTradingLimits(data, deps);
    case 'pause':
...
function handleStrategy(
  ...
  const strategy = String(data.payload.strategy ?? '');
  if (!strategy) {
    return { status: 400, body: { error: 'Missing payload.strategy' } };
  }

  const params = (data.payload.parameters ?? {}) as Record<string, number>;
  deps.db.setStrategyConfig(strategy, params);
  return { status: 200, body: { ok: true, proposalId: data.proposalId, strategy, parameters: params } };
}

function handleTradingLimits(
  data: ConfigRequest,
  deps: ConfigApiDeps,
): { status: number; body: Record<string, unknown> } {
  const limits = data.payload;
  if (!limits || Object.keys(limits).length === 0) {
    return { status: 400, body: { error: 'Missing payload (trading limit overrides)' } };
  }

  deps.db.setConfigValue('trading_limits', JSON.stringify(limits));
  return { status: 200, body: { ok: true, proposalId: data.proposalId, tradingLimits: limits } };
}
