/**
 * SolanaRegistryClient — on-chain discovery registry for agent/guardian discovery.
 *
 * Reads/writes to a Solana program that stores endpoint + TEE identity for each
 * registered node. Agents and guardians find each other by querying this registry
 * instead of relying on Telegram group messages or hardcoded BOOTSTRAP_GUARDIANS.
 */
...
export interface RegistryEntry {
  ...
  isActive: boolean;
  /** Solana pubkey (base58) of the PDA account. */
  owner: string;
}
...
export class SolanaRegistryClient {
  constructor(
  ...
  /** Send on-chain heartbeat to update last_heartbeat timestamp. */
