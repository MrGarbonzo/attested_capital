...
import { SolanaRegistryClient } from '../shared/solana-registry-client.js';
...
async function main(): Promise<void> {
  ...
  // ── Solana Discovery Registry ──────────────────────────────
  const registryProgramId = process.env.REGISTRY_PROGRAM_ID;
  let solanaRegistry: SolanaRegistryClient | undefined;
  ...
  if (registryProgramId) {
    ...
    // Derive a keypair for PDA lookups only — no on-chain writes.
    const derivationSig = await signer.sign('solana-registry-keypair-derivation');
    const guardianSolanaKeypair = Keypair.fromSeed(
      Buffer.from(derivationSig, 'base64').subarray(0, 32),
    );

    solanaRegistry = new SolanaRegistryClient(
      new Connection(solanaRpcUrl, 'confirmed'),
      guardianSolanaKeypair,
      new PublicKey(registryProgramId),
    );

    // Discover agent endpoint from registry (read-only, no SOL needed)
    try {
      const agents = await solanaRegistry.getAgents();
  ...
  // Solana registry re-discovery every 5 minutes (read-only, no SOL needed)
  if (solanaRegistry) {
    setInterval(async () => {
      try {
        const agents = await solanaRegistry!.getAgents();
